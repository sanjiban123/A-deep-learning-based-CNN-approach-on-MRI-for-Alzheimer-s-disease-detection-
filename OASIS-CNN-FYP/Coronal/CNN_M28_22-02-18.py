...
for train, test in kfold.split(X, y):
    ...
    model.add(Convolution2D(2, kernel_size=(3, 3), activation='relu', input_shape=input_shape))
    model.add(Flatten())
    ...
    history = model.fit(X[train], y[train],epochs=2, batch_size=5, verbose=1, validation_data=(X[test], y[test]), callbacks=[tbCallBack])
