#################################################################################################################
# 
#	FILE NAME   : CNN_M36_23-02-18.py
#	DESCRIPTION : Attempt 36 of basic CNN model. Run locally on Laptop.
...
# model.add(Convolution2D(12, kernel_size=(3, 3), activation='relu', input_shape=input_shape))
...
model.add(Convolution2D(6, kernel_size=(3, 3), activation='relu'))
model.add(MaxPooling2D(pool_size=(2,2)))
...
datagen.fit(X_train)
# X_batch, y_batch = datagen.flow(X_train, y_train, batch_size=8)
...
# history = model.fit(X_train, y_train,epochs=10, batch_size=5, verbose=1, validation_data=(X_test, y_test), callbacks=[tbCallBack])
