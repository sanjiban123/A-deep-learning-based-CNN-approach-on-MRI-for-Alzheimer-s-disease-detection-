#################################################################################################################
# 
#	FILE NAME   : CNN_M30_23-02-18.py
#	DESCRIPTION : Attempt 30 of basic CNN model. Run locally on Laptop. 1 Conv + 1 De.
...
model.add(Convolution2D(5, kernel_size=3, activation= "relu", input_shape=input_shape))
model.add(Convolution2D(5, kernel_size=3, activation= "relu"))
model.add(Convolution2D(5, kernel_size=3, activation= "relu"))
...
history = model.fit(X_train, y_train,epochs=20, batch_size=5, verbose=1, validation_data=(X_test, y_test), callbacks=[tbCallBack])
...
def PredictedValues(parameter_a, parameter_b):
    counter = 1
    file.write(" S. No. " + "|" + " Actual " + "|" + " Predicted \n")
    for ite in range(len(parameter_a)):
        file.write(str(counter) + "   |   " + str(parameter_a[ite]) + "   |   " + str(parameter_b[ite]) + "\n")  
        counter = counter + 1
    file.write("\n\n\n")
...
PredictedValues(y_train, y_pred_tr)
file.write('TESTING DATA CLASS LABELS:\n')
PredictedValues(y_test, y_pred_te)
