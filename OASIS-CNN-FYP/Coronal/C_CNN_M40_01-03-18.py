#################################################################################################################
# 
#	FILE NAME   : C_CNN_M40_01-03-18.py
#	DESCRIPTION : Attempt 5 of basic CNN model. Run locally on Laptop.
...
model.add(Convolution2D(12, kernel_size=(39, 39), activation='relu', input_shape=input_shape))
# model.add(Convolution2D(8, kernel_size=(39, 39), activation='relu'))
model.add(Convolution2D(4, kernel_size=(39, 39), activation='relu'))
model.add(Convolution2D(2, kernel_size=(39, 39), activation='relu'))
...
history = model.fit(X_train, y_train,epochs=3, batch_size=5, verbose=1, validation_data=(X_test, y_test), callbacks=[tbCallBack])
...
with open('Coronal Outputs/' + str(modelName) + '/' + str(modelName) + "_JSONArchitecture.json", "w") as json_file:
...
model.save('Coronal Outputs/' + str(modelName) + '/' + str(modelName) + "_ModelDetails.h5")

# DELETING EXISTING MODEL
del model

# LOADING MODEL FROM THE SAVED H5 FILE
model = load_model('Coronal Outputs/' + str(modelName) + '/' + str(modelName) + "_ModelDetails.h5")

##################################################################################################################
# SAVING MODEL ARCHITECTURE AND TRAINING AND VALIDATION DATA FROM HISTORY FUNCTION AS IMAGES
##################################################################################################################

plot_model(model, to_file='Coronal Outputs/' + str(modelName) + '/' + str(modelName) + "_modelPlot.png", show_shapes=True, show_layer_names=True)
...
plt.savefig('Coronal Outputs/' + str(modelName) + '/' + str(modelName) + '_Accuracy_Graph.png', dpi=150)
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Test'], loc='upper left')
plt.savefig('Coronal Outputs/' + str(modelName) + '/' + str(modelName) + '_Loss_Graph.png', dpi=150)
...
file = open('Coronal Outputs/' + str(modelName) + '/' + str(modelName) + "_Metrics.txt", "w")
...
def MetricCalc(parameter_a, parameter_b, dataset_type):
    ...
    plt.savefig('Coronal Outputs/' + str(modelName) + '/' + str(modelName) + '_CNFMatrix' + str(dataset_type) + '.png', dpi=150)
...
##################################################################################################################
# CONVERTING OUTPUT LABELS BACK TO SIMPLE CLASS Coronal OUTPUTS FROM CATEGORICAL DATA
##################################################################################################################
...
file = open('Coronal Outputs/' + str(modelName) + '/' + str(modelName) + "_Metrics.txt", "a")
file.write('FORMULAS:\n')
