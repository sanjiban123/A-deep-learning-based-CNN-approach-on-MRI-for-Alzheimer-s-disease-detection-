#################################################################################################################
# 
#	FILE NAME   : C_CNN_M42_07-03-18.py
#	DESCRIPTION : Attempt 41 of basic CNN model. Run locally on Laptop. Uses ImageDataGen with the CNN module that obtained 80%
...
image_datagen_var = 1
do_random = 1
set_var_by_rescaling_target_classes = 0
...
if image_datagen_var == 0:
    ...
    history = model.fit_generator(datagen.flow(X_train, y_train, batch_size=47), steps_per_epoch=15*len(X_train), epochs=5, verbose=1, validation_data=(X_test, y_test), callbacks=[tbCallBack])
