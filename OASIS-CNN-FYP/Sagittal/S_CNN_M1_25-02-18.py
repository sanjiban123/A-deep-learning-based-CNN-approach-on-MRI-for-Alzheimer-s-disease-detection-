#################################################################################################################
# 
#	FILE NAME   : S_CNN_M1_25-02-18.py
#	DESCRIPTION : Attempt 1 of basic CNN model on Sagittal view. Run locally on Laptop.
...
exec(open('sagittal_config.py').read())
exec(open('sagittal_randomizer.py').read())
...
tbCallBack = keras.callbacks.TensorBoard(log_dir='./Sagittal Outputs/' + str(modelName), histogram_freq=0, write_graph=True, write_images=True)

# INITIALIZING MODEL
model = Sequential()

# CONVOLUTIONAL LAYERS
model.add(Convolution2D(4, kernel_size=(5, 5), activation='relu', input_shape=input_shape))
...
exec(open('sagittal_model_save.py').read())
...
exec(open('sagittal_metric_functions.py').read())